"""
Agent 2: Mistral Small Latest (via Mistral API)
Generates comprehensive Russian training data - Batch 1
"""
import json
import requests
import os
...
API_KEY = "3f3CxheWZbnuh8SrVILhg88BoArPpZex"
...
Сделайте контент образовательным, разнообразным и полезным для обучения ИИ-ассистента. НЕ повторяйте контент."""
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"Agent 2 Error: {response.status_code} - {response.text}")
            return []
        ...
        if start >= 0 and end > start:
            ...
            print(f"Agent 2 (Mistral Small): Generated {len(data)} examples")
            return data
        return []
    except Exception as e:
        print(f"Agent 2 Exception: {e}")
        return []
...
if __name__ == "__main__":
    ...
    if data:
        output_file = output_dir / f"russian_agent2_batch{batch_id}.json"
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
        print(f"Agent 2: Saved to {output_file}")
    else:
        print("Agent 2: No data generated")
